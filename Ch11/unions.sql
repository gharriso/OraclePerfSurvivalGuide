/* Formatted on 2008/11/29 21:33 (Formatter Plus v4.8.7) */
SPOOL unions
SET serveroutput on

ALTER SESSION SET tracefile_identifier=unions;
...
CREATE     TABLE microsoft_customers
    ...
    cust_eff_to                    DATE,
    cust_valid                     VARCHAR2(1)) nologging;
/* Formatted on 2008/11/29 21:58 (Formatter Plus v4.8.7) */
/* Formatted on 2008/11/29 22:01 (Formatter Plus v4.8.7) */
BEGIN
   FOR i IN 1 .. 3
   LOOP
...
EXPLAIN PLAN FOR
      ...
      UNION
      SELECT cust_first_name cust_first_name, cust_last_name,
             cust_year_of_birth
...
EXPLAIN PLAN FOR
/* Formatted on 2008/11/29 22:10 (Formatter Plus v4.8.7) */
...
DECLARE  /* gh1 union notall */
   CURSOR c1
   IS
      SELECT                                           /* gh1 union notall */
             ' gh1 union notall' t,cust_first_name  cust_first_name, cust_last_name,
             cust_year_of_birth
        FROM microsoft_customers gh1 
      UNION
      SELECT ' gh1 union notall' t,cust_first_name cust_first_name, cust_last_name,
             cust_year_of_birth
        FROM google_customers;
...
DECLARE   /* gh1 union all */
   CURSOR c1
   IS
      SELECT -- gh1 union all                                              
             ' gh1 union all' t,cust_first_name  , cust_last_name,
             cust_year_of_birth
        FROM microsoft_customers gh1 
      UNION ALL
      SELECT ' gh1 union all' t,cust_first_name cust_first_name, cust_last_name,
             cust_year_of_birth
        FROM google_customers;
