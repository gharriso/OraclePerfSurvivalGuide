create or replace view latch_delta_view as 
WITH wait_times AS 
  (SELECT CASE
            WHEN (stat_name LIKE '%latch%'  or stat_name LIKE '%mutex%' or stat_name like 'cursor:%') 
            then stat_name
...
SELECT wait_type, SUM(total_waits) total_waits,
       ...
       round(sum(time_waited_ms)*100/sum(sum(time_waited_ms))   over(),2) pct_time,
