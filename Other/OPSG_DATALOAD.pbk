CREATE OR REPLACE PACKAGE BODY OPSG_DATALOAD
IS
   ...
   BEGIN
      ...
      BULK COLLECT INTO g_first_names
                 FROM hr.employees
      UNION
   ...
   PROCEDURE clone_job_history (
      v_orig_employee_id   NUMBER,
      p_new_employee_id    NUMBER
   )
   IS
      v_offset   NUMBER;
   BEGIN
   ...
   PROCEDURE clone_employee (
      p_new_employee_id   NUMBER,
      p_employee_row      hr.employees%ROWTYPE
   )
   IS
   ...
   IS
      ...
      v_new_employee_id   NUMBER;
   BEGIN
      FOR i IN 1 .. LEAST (g_employees.COUNT, p_limit)
      LOOP
   ...
   BEGIN
      ...
      IF (g_first_names.COUNT = 0)
      THEN
         populatenames ();
      END IF;

      WHILE (l_count < employee_count)
      LOOP
         l_count := l_count + addbatch (employee_count);
      END LOOP;
   END;

   FUNCTION createtime (p_time_id DATE, source_date DATE)
      RETURN sh.times%ROWTYPE
   IS
   ...
   BEGIN
      ...
      new_row.week_ending_day := p_time_id + 7 - new_row.day_number_in_week;
      new_row.week_ending_day_id := new_row.week_ending_day_id + days_offset;
      ...
      new_row.calendar_month_id := new_row.calendar_month_id + days_offset;
      ...
      new_row.end_of_cal_quarter :=
         TO_DATE (TO_CHAR (new_row.end_of_cal_quarter, 'DD-MON-') || thisyear,
                  'DD-MON-YYYY'
                 );
   ...
   FUNCTION getsourcedate (this_date DATE, source_year NUMBER)
      RETURN DATE
   IS
   ...
   BEGIN
      FOR r IN (SELECT partition_name
                 ...
                 WHERE table_owner = 'SH' AND table_name = table_name)
      LOOP
      ...
      LOOP
         partition_name :=
                      UPPER (table_name) || '_Q' || i || '_'
                      || partition_year;

         IF NOT part_list.EXISTS (partition_name)
         THEN
            end_of_qtr :=
               CASE i
                  WHEN 1
...
INSERT INTO sh.costs
            (prod_id, time_id, promo_id, channel_id, unit_cost,
             unit_price
            ) select 
            prod_id, v_this_date, promo_id, channel_id, unit_cost,
             unit_price   FROM sh.costs
             WHERE time_id = v_source_date;
