...
CREATE OR REPLACE FUNCTION emc2_pl_number (m NUMBER)
   RETURN NUMBER
IS
...
BEGIN
   ...
   FOR i IN 1 .. v_max_mass
   LOOP
      v_out := emc2_pl_number (i);
   END LOOP;

   DBMS_OUTPUT.put_line (v_out);
   DBMS_OUTPUT.put_line (
      'PLSQL (NUMBER): ' || (DBMS_UTILITY.get_time () - v_start));
END;
/* Formatted on 14/12/2008 10:40:09 AM (QP5 v5.120.811.25008) */
/
