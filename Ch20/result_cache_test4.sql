...
BEGIN
   sys.DBMS_STATS.gather_table_stats(OWNNAME => USER, TABNAME => 'SALES');
      sys.DBMS_STATS.gather_table_stats(OWNNAME => USER, TABNAME => 'SALES_ARCHIVE');
...
CREATE PROCEDURE load_rscache IS
...
BEGIN
   ...
   FOR i IN 1 .. 1200 LOOP
      v_cust_id := t_cust_ids(ROUND(DBMS_RANDOM.VALUE(1, 1000)));
      v_prod_id := t_prod_ids(ROUND(DBMS_RANDOM.VALUE(1, 50)));
      BEGIN
         FOR r IN c1 LOOP
            NULL;
         END LOOP;
...
/* Formatted on 3/03/2009 11:48:50 AM (QP5 v5.120.811.25008) */
