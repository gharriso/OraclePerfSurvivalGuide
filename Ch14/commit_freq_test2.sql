...
CREATE TABLE commit_test_data (commit_settings varchar2(100), commit_interval NUMBER, elapsed        NUMBER);
...
CREATE OR REPLACE PACKAGE commit_freq_test IS
   ...
   PROCEDURE testit (commit_logging varchar2  , commit_wait varchar2 ) ; 
END;
/
...
CREATE OR REPLACE PACKAGE BODY commit_freq_test IS
   ...
   PROCEDURE testit (commit_logging varchar2  , commit_wait varchar2 ) IS
   ...
   BEGIN
      ...
      v_commit_int := 1;
      execute immediate 'alter system set commit_wait='||commit_wait||' scope=memory';   
      execute immediate 'alter system set commit_logging='||commit_logging||' scope=memory';   
      WHILE v_commit_int < 150  LOOP
         v_ela := testins(v_commit_int);

         INSERT INTO commit_test_data
         VALUES (commit_wait||','||commit_logging,v_commit_int, v_ela);
...
BEGIN
commit_freq_test.init; 
   FOR i IN 1 .. 3 LOOP
      
      commit_freq_test.testit('IMMEDIATE','NOWAIT');
      commit_freq_test.testit('BATCH','NOWAIT');
      commit_freq_test.testit('IMMEDIATE','WAIT');
      
   END LOOP;
END;
/
select * from commit_test_data; 
