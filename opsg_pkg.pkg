...
END;
/
CREATE OR REPLACE PACKAGE BODY opsg_pkg
/* Formatted on 7/03/2011 9:34:28 AM (QP5 v5.160) */
IS
   ...
   WITH time_model AS
     (
        SELECT /*+ materialize */
               SUM
                  (CASE
                      WHEN stat_name IN (''DB CPU'', ''background cpu time'')
                         THEN VALUE
                   END
                  ) AS cpu_time,
               SUM
   ...
   SELECT   event, microseconds, total_waits,
    ...
    PROCEDURE save_to_sysstat_previous
    IS
    BEGIN
    ...
    BEGIN
       ...
       -- DBMS_OUTPUT.put_line (g_current_waits.COUNT || ' g_current waits');
    END;

    PROCEDURE report_current
    IS
    ...
    BEGIN
        ...
        WHILE v_stat_name IS NOT NULL
        LOOP
            g_time_model_data.EXTEND;
            v_stat_line :=
                opsg_time_model_line_typ (
                    v_start_time,
                    g_current_timestamp,
                    v_stat_name,
                    g_current_waits (v_stat_name),
    ...
    BEGIN
        ...
        v_interval_secs := (g_current_timestamp - v_start_time) * 24 * 3600;
        v_stat_name := g_current_waits.FIRST;
       -- DBMS_OUTPUT.put_line ('Report Delta first wait=' || v_stat_name);
        ...
        LOOP
            ...
            IF g_prev_waits.EXISTS (v_stat_name)
            THEN
                --DBMS_OUTPUT.put_line ('Previous exists');
                v_stat_line :=
                    opsg_time_model_line_typ (
                        v_start_time,
                        g_current_timestamp,
                        ...
                        / v_interval_secs);
            ELSE
                --DBMS_OUTPUT.put_line ('Previous not exists');
                v_stat_line :=
                    opsg_time_model_line_typ (
                        v_start_time,
                        g_current_timestamp,
            ...
            v_stat_name := g_current_waits.NEXT (v_stat_name);
        --DBMS_OUTPUT.put_line ('Next wait=' || v_stat_name);
        END LOOP;

        --DBMS_OUTPUT.put_line ('End report delta');
    END;
    ...
    BEGIN
        ...
        load_current_waits;
        --DBMS_OUTPUT.put_line ('Load Current Waits complete');


        IF g_prev_waits.COUNT = 0
        THEN
            report_current;
            --DBMS_OUTPUT.put_line ('REport current complete');
        ELSE
            report_delta;
            --DBMS_OUTPUT.put_line ('Report delta complete');
        END IF;

        save_to_previous;
        --DBMS_OUTPUT.put_line ('Save to previous complete');
        /*DBMS_OUTPUT.put_line (
               'Returning '
            || g_time_model_data.COUNT
            || ' g_time_model_data rows'); */
        RETURN (g_time_model_data);
    END;


    PROCEDURE report_sysstat_current
    IS
        v_stat_name       VARCHAR2 (1000);
        v_start_time      DATE;
        v_interval_secs   NUMBER;
    ...
    BEGIN
        ...
        LOOP
            ...
            v_stat_line :=
                opsg_sysstat_line_typ (
                    v_start_time,
                    g_current_timestamp_ss,
                    v_stat_name,
    ...
    PROCEDURE report_sysstat_delta
    IS
        v_stat_name       VARCHAR2 (1000);
        v_start_time      DATE;
        v_interval_secs   NUMBER;
    ...
    BEGIN
        ...
        LOOP
            ...
            v_stat_line :=
                opsg_sysstat_line_typ (v_start_time,
                                       g_current_timestamp_ss,
                                       v_stat_name,
                                       v_delta,
    ...
    PROCEDURE load_current_sysstat
    IS
        v_names   DBMS_SQL.varchar2_table;
        v_vals    DBMS_SQL.number_table;
    BEGIN
        SELECT name, VALUE
          BULK COLLECT INTO v_names, v_vals
          FROM sys.v_$sysstat;
    ...
    PROCEDURE load_current_servstat
    IS
        CURSOR servstat_csr
        IS
            SELECT startup_time,
                   ...
                   stat_name,
                   VALUE
              FROM gv$service_stats JOIN gv$instance USING (inst_id)
    ...
    BEGIN
        ...
        FOR r IN servstat_csr
        LOOP
            v_stat_line :=
                opsg_servstat_line_typ (r.startup_time,
                                        SYSDATE,
                                        r.inst_id,
                                        r.service_name,
                                        r.stat_name,
    ...
    PROCEDURE report_current_servstat
    IS
    BEGIN
        g_servstat_data := g_current_servstat;
    END;

    PROCEDURE save_to_previous_servstat
    IS
    BEGIN
    ...
    PROCEDURE report_delta_servstat
    IS
